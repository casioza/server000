...
int guild_leave(struct map_session_data* sd, int guild_id, int account_id, int char_id, const char* mes)
{
	...
	if(sd->status.account_id!=account_id ||
		...
		((agit_flag || agit2_flag || agit3_flag) && map[sd->bl.m].flag.gvg_castle))
		return 0;
...
int guild_delalliance(struct map_session_data *sd,int guild_id,int flag)
{
	...
	if(agit_flag || agit2_flag || agit3_flag)	{	// Disable alliance breaking during woe [Valaris]
...
int guild_opposition(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	for(i=0;i<MAX_GUILDALLIANCE;i++){	// すでに関係を持っているか確認
		if(g->alliance[i].guild_id==tsd->status.guild_id){
			...
			if(agit_flag || agit2_flag || agit3_flag) // Prevent the changing of alliances to oppositions during WoE..
				return 0;
