...
{
	if(agit3_flag==0) return 0;      // Agit3 already End.
	agit3_flag=0;
	guild_agit3_end();
	return 0;
}
...
{ 
	...
	if( st->sleep.tick == 0 ) { st->state = RERUNLINE;      st->sleep.tick = ticks; } 
	else { st->state = RUN; st->sleep.tick = 0; } 
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(agitend,""),
	BUILDIN_DEF(agitstart3,""),	// <AgitStart3>
	BUILDIN_DEF(agitend3,""), // <Agitend3
