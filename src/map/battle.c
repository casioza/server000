...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		if( sd )
		{
			...
			if (skill_num && (i = pc_skillatk_bonus(sd, skill_num)))
				ATK_ADDRATE(i);
			if((i = skill_get_index(skill_num)) && skill_db[i].damage_modifier != 0)
			ATK_ADDRATE(skill_db[i].damage_modifier-100);
...
#define MATK_ADD( a ) { ad.damage+= a; }
		...
		if(sd) {
			...
			if((i = skill_get_index(skill_num)) && skill_db[i].damage_modifier != 0)
				ad.damage += ad.damage*(skill_db[i].damage_modifier-100)/100;
			//Ignore Defense?
			if (!flag.imdef && (
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	if (sd)
	{
		...
		if((i = skill_get_index(skill_num)) && skill_db[i].damage_modifier != 0)
		md.damage += md.damage*(skill_db[i].damage_modifier-100)/100;
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{	//Checks on target master
		...
		case BL_MOB:
		{
			...
			if( !((agit_flag || agit2_flag || agit3_flag) && map[m].flag.gvg_castle) && md->guardian_data && md->guardian_data->guild_id )
	...
	{	//Checks on source master
		case BL_PC:
		{
			...
			if( s_bl != t_bl )
			{
				...
				else if( sd->state.pvp && (sd->state.pvp == ((TBL_PC*)t_bl)->state.pvp) )
					state |= BCT_ENEMY;
			}
...
// BattleGround Settings
	...
	// PvP Area Settings
	{ "pvp_area_deathmath",					&battle_config.pvp_area_deathmath,				 0,		0,		1,				},		
};
