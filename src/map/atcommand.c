...
/*==========================================
 * Recall Characters ไม่มีผลกับ autotrade
 *------------------------------------------*/
ACMD_FUNC(recallsome)
{
	struct map_session_data* pl_sd;
	struct s_mapiterator* iter;
	int num_failed;
...
/*========================================== 
* @afk by Rad, built on by Tubby :P 
...
{ 
	...
	clif_displaymessage(fd,"ไม่สามารถใช้คำสั่งนี้ในแมพ pvp/gvg/battleground. ได้"); 
	return -1; 
	}
	if( map[sd->bl.m].flag.autotrade == battle_config.autotrade_mapflag ) 
	{
	...
	chat_createpcchat(sd, "(AFK) ไม่อยู่ ~", "", 1, 1); 
	trade_tradeack(sd,4); 
	sd->state.autotrade = 1; 
	...
	chat_createpcchat(sd, "(@afk) พักผ่อน ~", "", 1, 1); 
	trade_tradeack(sd,4); 
	sd->state.autotrade = 1; 
...
{ 
	...
	clif_authfail_fd(fd, 15); 
		...
		clif_displaymessage(fd, "ไม่สามารถใช้คำสั่ง @afk ในแผนที่นี้ได้."); 
	return 0; 
}

/*========================================== 
* @sleep by exsoulionzz :P 
* not map pvp gvg
*------------------------------------------*/ 
int atcommand_sleep(const int fd, struct map_session_data* sd, const char* command, const char* message) 
{ 
	if (map[sd->bl.m].flag.pvp || map[sd->bl.m].flag.gvg || map[sd->bl.m].flag.battleground) {
	clif_displaymessage(fd, "ไม่สามารถใช้ @sleep ในแมพ PVP และ GVG ได้"); //Can?t use this command in PvP and GvG.
	return -1;
	} 
...
{ 
	...
	if(g==NULL) 
	{ 
		clif_displaymessage(fd, "คุณยังไม่มี Guild ค่ะ."); 
		return -1; 
	} 
		if (strcmp(g->master, sd->status.name) != 0) 
	{ 
  	 	clif_displaymessage(fd, "คุณไม่ใช่ Guild Master ค่ะ."); 
	   	return -1; 
	} 
...
struct guild *g; 
		...
		if(g==NULL) 
	{ 
  		clif_displaymessage(fd, "คุณยังไม่มี Guild."); 
  		return -1; 
	} 
		if (strcmp(g->master, sd->status.name) == 0) 
	{ 
  		clif_displaymessage(fd, "คุณเป็น Guild Master คำสั่งนี้ไม่อาจใช้งานได้."); 
  		return -1; 
	} 
		if(!message || !*message) 
		message = "usage @leaveguild without a reason."; 
		intif_guild_leave(sd->status.guild_id, sd->status.account_id, sd->status.char_id,0, message); 
		return 0; 
}
...
ACMD_FUNC(itemmap)
{
	...
	clif_displaymessage(fd, "คำสั่งใช้งานดังนี้ (@itemmap ชื่อแมพ เลขไอเท็ม จำนวน)");
		return -1;
	}
	if (number <= 0)
	number = 1;
	...
	if ((flag = pc_additem(pl_sd, &item_tmp, get_count)))
		...
		count ++;
	clif_displaymessage(pl_sd->fd, "[ Admin System ] : คุณได้รับไอเท็มจาก GM ค่ะ."); // Message for all players on the map (can't use "mercy.." because we can use a Support or a Attack skill with @skillmap)
			}
		}
	}
}
	snprintf(atcmd_output, sizeof(atcmd_output) ,"ผู้เล่นได้รับไอเท็มจากคุณ : %d คน", count);
		clif_displaymessage(fd, atcmd_output);
			return 0;
}
...
AtCommandInfo atcommand_info[] = {
	...
	{ "breakguild",        40,40,     atcommand_breakguild },
	{ "leaveguild",        40,40,     atcommand_leaveguild },
	{ "itemmap",           99,99,     atcommand_itemmap },
	{ "maprecall",         90,90,     atcommand_maprecall },
	{ "recallsome",        90,90,     atcommand_recallsome },
	{ "reloadnpcscript",      99,99,     atcommand_reloadnpcscript },
	{ "sleep",      40,40,     atcommand_sleep },
	{ "autoattack",	40,40,	atcommand_autoattack	}
};
