...
int guild_calcinfo(struct guild *g)
{
	...
	g->max_member = guild_max_members + guild_checkskill(g, GD_EXTENSION) * guild_extension_bonus; //Guild Extention skill - currently adds 6 to max per skill lv. 
		if(g->max_member > MAX_GUILD) 
	{ 
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	g->max_member=guild_max_members;
